...
import com.example.e_finance.util.DisplayUtil;
...
public class BubbleProgressView extends View {
    ...
    private int mColorProgressStr = Color.BLACK;//进度条的进度文字的颜色
    private float mProgressHeight = DisplayUtil.dp2px(getContext(),3);//进度条的厚度
    private float mProgress=0;//进度条的进度
    private float mBubbleTriangleHeight = DisplayUtil.dp2px(getContext(),4);//气泡底部小三角高度
    private float mBubbleRectRound = DisplayUtil.dp2px(getContext(),5);//气泡的圆角
    private String mProgressStr = "";//显示进度的字符串
    private float mTextSize = DisplayUtil.sp2px(getContext(),12);//进度条文字大小
    private Paint.FontMetricsInt mFontMetricsInt;
    private float mProgressStrMargin = DisplayUtil.dp2px(getContext(),5);//气泡的边距
    ...
    private void init() {
        ...
        mColorProgressBg = getResources().getColor(R.color.white);
        mColorProgress = getResources().getColor(R.color.yellowalpha);
        mPaintBubble.setColor(Color.WHITE);//设置气泡的颜色
    ...
    private void drawBubble(Canvas canvas) {
        ...
        //      进度所在的横坐标 - 底部三角形高度 - 矩形圆角的七分之五(不减去圆角的七分之五的话显得不够圆润)，
        ...
        RectF rectF = new RectF(p[0]-mBubbleTriangleHeight-(5*mBubbleRectRound)/7-(mProgress*width),p[1]-mBubbleTriangleHeight-mProgressHeight-height,p[0]+mBubbleTriangleHeight+mBubbleRectRound/2+((1-mProgress)*width),p[1]-mBubbleTriangleHeight-mProgressHeight);
    ...
    public void setProgress(float progress,String str) {
        this.mProgress = progress;
        mProgressStr = str;
        invalidate();//设置完进度进行重绘
    }

    public void setmTextSize(int mTextSizeDp) {
        this.mTextSize = DisplayUtil.sp2px(getContext(),mTextSizeDp);
    }
